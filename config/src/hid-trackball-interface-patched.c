 ...
 *
 * Patched copy of zmk-hid-trackball-interface/src/hid-trackball-interface.c
 * Fix: zmk_keymap_layer_activate/deactivate now require a second bool argument
 * (added in ZMK PR #2717).
 */
...
static void activate_automouse_layer() {
    zmk_keymap_layer_activate(config.automouse_layer, false);
...
static void deactivate_automouse_layer(struct k_work *item) {
    if (zmk_keymap_layer_active(config.automouse_layer)) {
        zmk_keymap_layer_deactivate(config.automouse_layer, false);
